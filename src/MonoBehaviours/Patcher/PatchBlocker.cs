...
class PatchBlocker : MonoBehaviour
{
    ...
    public void Start()
    {
        if (type == Type.FLOOR)
        {
            ...
            Transform t = actualBlocker.Find("Active").transform;
            Transform t2 = gameObject.Find("Active").transform;
            ...
            PlayMakerFSM blockerFsm = actualBlocker.LocateMyFSM("quake_floor");
            if (blockerFsm.GetState("Init").Fsm == null)
            {
                blockerFsm.Preprocess();
            }
            blockerFsm.SetAttr("fsmTemplate", (FsmTemplate) null);
            FsmVariables blockerFsmVars = blockerFsm.FsmVariables;
            ...
            ActivateGameObject agoAction = new ActivateGameObject
            {
